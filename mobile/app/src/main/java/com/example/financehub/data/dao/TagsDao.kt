...
interface TagsDao {
    ...
    @Query("SELECT * FROM tags WHERE pendingSync = 1")
    suspend fun getPendingSyncTags(): List<Tags>

    @Query("SELECT * FROM tags WHERE serverId = :serverId")
    suspend fun getTagByServerId(serverId: String): Tags?
    ...
    @Query("""
        UPDATE tags 
        SET tag = :name, monthlyAmount = :monthlyAmount, currentMonth = :currentMonth, currentYear = :currentYear,
            syncUpdatedAt = :updatedAt, lastSyncedAt = :lastSyncedAt
        WHERE tagID = :tagId
    """)
    ...
    @Query("""
        UPDATE tags 
        SET pendingSync = 1, syncOperation = :operation, syncUpdatedAt = :updatedAt
        WHERE tagID = :tagId
    """)
    suspend fun markForSync(tagId: Int, operation: String, updatedAt: Long)
