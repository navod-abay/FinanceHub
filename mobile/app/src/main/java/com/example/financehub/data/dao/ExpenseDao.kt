...
""")
    ...
    @Query("""
        UPDATE expenses 
        SET title = :title, amount = :amount, year = :year, month = :month, date = :date, 
    ...
    @Query("""
        ...
        WHERE expenseID = :expenseId
    """)
    suspend fun markForSync(expenseId: Int, operation: String, updatedAt: Long)

    @Query("DELETE FROM expenses WHERE createdAt < :cutoffTime")
    suspend fun deleteOldExpenses(cutoffTime: Long)
}
